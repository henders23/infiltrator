...
export const WALL = 1;
export const DOOR = 2;
export type TileKind = typeof FLOOR | typeof WALL | typeof DOOR;
...
export class Grid {
  ...
  /** Floors and doors can be walked; walls and out-of-bounds cannot. */
  isWalkable(x: number, y: number): boolean {
    return this.inBounds(x, y) && this.get(x, y) !== WALL;
  }
}
...
export function gridFromAscii(rows: string[]): Grid {
  ...
  for (let y = 0; y < height; y++) {
    ...
    for (let x = 0; x < width; x++) {
      const ch = row[x] ?? '.';
      grid.set(x, y, ch === '#' ? WALL : ch === '+' ? DOOR : FLOOR);
    }
  }
  return grid;
}
